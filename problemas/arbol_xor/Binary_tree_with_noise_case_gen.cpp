...
auto NaryTree(int N, int depth) -> vector<array<int, 2>> { // 0-indexed
...
auto AddRandomNoiseToTree(vector<array<int, 2>> V, int K) -> vector<array<int, 2>> {
  int N = (int)V.size(); N++;
  for (int i = N; i < N + K; i++) {
    int j = (rng() % i);
    V.push_back({j, i});
...
int main() {
  cout << 11 << endl;
  for (int i = 0; i < 10; i++) {
    vector<array<int, 2>> dummy = PermuteLabels(100, AddRandomNoiseToTree(NaryTree(2, 5), 37));
    testcase X;
    for (auto [a, b] : dummy) X.v.push_back(a), X.u.push_back(b), X.w.push_back(rng()%(1000000000));
    X.N = 100;
    X.print();
  }
  vector<array<int, 2>> dummy = PermuteLabels(1000, AddRandomNoiseToTree(NaryTree(2, 8), 489));
  testcase X;
  for (auto [a, b] : dummy) X.v.push_back(a), X.u.push_back(b), X.w.push_back(rng()%(1000000000));
  X.N = 1000;
  X.print();
